...
userrole = ENUM("admin", "staff", name="userrole", create_type=False)
finishtype = ENUM("matte","gloss","satin","textured","woodgrain","metallic","other", name="finishtype", create_type=False)
uploadstatus = ENUM("pending","processing","completed","failed", name="uploadstatus", create_type=False)
feedbacktype = ENUM("confirmed","rejected","corrected", name="feedbacktype", create_type=False)


def upgrade() -> None:
    conn = op.get_bind()
    conn.execute(sa.text("CREATE TYPE IF NOT EXISTS userrole AS ENUM ('admin','staff')")) if False else None
    for name, vals in [
        ("userrole", "('admin','staff')"),
        ("finishtype", "('matte','gloss','satin','textured','woodgrain','metallic','other')"),
        ("uploadstatus", "('pending','processing','completed','failed')"),
        ("feedbacktype", "('confirmed','rejected','corrected')"),
    ]:
        exists = conn.execute(sa.text("SELECT 1 FROM pg_type WHERE typname=:n"), {"n": name}).fetchone()
        if not exists:
    ...
    op.create_table("products",
        ...
        sa.Column("finish", finishtype, nullable=True),
